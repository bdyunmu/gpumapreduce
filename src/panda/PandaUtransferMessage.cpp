...
Code Name: Panda 0.44
File: PandaUtransferMessage.cpp
Time: 2016-04-18
Developer: Hui Li (lihui@indiana.edu)
This is the source code for Panda, a MapReduce Framework on GPUs and CPUs.
*/

#include <mpi.h>
#include <panda/PandaMessageIORequest.h>
#include <panda/PandaUtransferMessage.h>
#include <cstring>
#include "Panda.h"
...
namespace panda
{
	...
	PandaUtransferMessage::PandaUtransferMessage() : PandaMessage(false)
	{
	
	
	}//void

	PandaUtransferMessage::PandaUtransferMessage(const bool pCopySendData) : PandaMessage(pCopySendData)
	{
		//copySendData  = pCopySendData;
	}//void

	PandaUtransferMessage::~PandaUtransferMessage()
	{

	}//void

	void PandaUtransferMessage::setPnc(panda_node_context *pnc)
	{
		this->pnc = pnc;
	}//void

	void PandaUtransferMessage::MsgInit(){
	...
	void PandaUtransferMessage::MsgFinalize(){
	
	}//void

	
	oscpp::AsyncIORequest * PandaUtransferMessage::MsgFinish()
	{
		return sendTo(commRank, NULL, NULL, NULL, -1, -1, -1);
	}//oscpp
		
	void PandaUtransferMessage::run()
	{	//void
		
		ShowLog("start PandaUtransferMessage thread, randk id [%d/%d]", commRank, commSize);
	...
	oscpp::AsyncIORequest * PandaUtransferMessage::sendTo(const int rank,
		void * const keys,
		void * const vals,
	...
	bool PandaUtransferMessage::pollUnsent()
	{
	...
	void PandaUtransferMessage::pollPending()
	{
		if (pendingIO.empty()) 
			return;
	...
	void PandaUtransferMessage::pollSends()
	{
	...
	void PandaUtransferMessage::PandaAddRecvedBucket(char *keyRecv, char *valRecv, int *keyPosKeySizeValPosValSize, int keyBuffSize, int valBuffSize, int maxlen)
	{	
	...
	void PandaUtransferMessage::poll(int & finishedWorkers,
